			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="sexo" title="${message(code: 'contacto.sexo.label', default: 'Sexo')}" />
					
						<g:sortableColumn property="edad" title="${message(code: 'contacto.edad.label', default: 'Edad')}" />
				...
				<g:each in="${contactoInstanceList}" status="i" var="contactoInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${fieldValue(bean: contactoInstance, field: "sexo")}</td>
					
						<td>${fieldValue(bean: contactoInstance, field: "edad")}</td>
